...
import { seleccionClasif } from "./seleccionClasif";
...
const CLASIFMATERIALES_TIMESTAMP = "clasifMateriales.timeStampFiltrado";
const MEDIA_CHANGE = "ui.media.timeStamp";
const SCREEN = "screen.timeStamp";
const UPDATE_FIELD = "clasifMateriales.updateFieldTimeStamp";
export class listaClasifMateriales extends connect(store, CLASIFMATERIALES_TIMESTAMP, MEDIA_CHANGE, SCREEN, UPDATE_FIELD)(LitElement) {
    constructor() {
        super();
        this.value = -1;
        this.area = "body";
    ...
    render() {
        return html`
            <seleccion-clasif class="divFijo"></seleccion-clasif>
            ${this.dibujaGrilla()}
        `;
    }
            ...
            return html`
                <div class="grid grillaAlign" id="comboClasif">
                    ${this.items.map((item) => {
                        if (item.Id != 0) {
                            return html`
                            <div tabindex="0" class="row" .item="${item}" .field="${"Descrip"}" @click="${this.seleccionar}" @keydown="${this.onkeyDownRow}">
                                <div>${item.Descrip}</div>
                                <input type="text" class="inputABM" .value="${item.Descrip}" @keyup="${this.onkeyUp}" @blur="${this.blur}"></input>
                            </div>
                            `;
                        }
                    })}
    ...
    stateChanged(state, name) {
        if (name == SCREEN || name == MEDIA_CHANGE) {
            ...
            const SeMuestraEnUnasDeEstasPantallas = "-clasifMateriales-".indexOf("-" + state.screen.name + "-") != -1;
        ...
        if (name == CLASIFMATERIALES_TIMESTAMP) {
            this.items = state.clasifMateriales.filtrado;
            this.update();
        }
...
window.customElements.define("lista-clasifmateriales", listaClasifMateriales);
