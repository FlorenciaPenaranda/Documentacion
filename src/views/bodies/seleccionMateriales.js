...
export class seleccionMateriales extends connect(store, BLANQUEAR_TIMESTAMP, FILTRO_TIMESTAMP, CLASIFMATERIALES_TIMESTAMP)(LitElement) {
    constructor() {
        super();
        this.afiliado = {};
        this.clasifItems = [];
    ...
    render() {
        ...
        <div class="grid fit">
            <div class="select">
                ...
                <Select Id="clasif" Name="clasif" >
                    ${this.clasifItems.map((item) => {
                        return html`<option value="${item.Id}">${item.Descrip}</option>`;
                    })}
                </Select>
            </div>
            <div class="input">
    ...
    buscar(e) {
        ...
        const idClasif = this.shadowRoot.querySelector("#comboClasif").value;

        filtros = {
            clasif: idClasif,
            descrip: aBuscar,
        };
        store.dispatch(buscar(filtros));
    }
...
window.customElements.define("seleccion-materiales", seleccionMateriales);
